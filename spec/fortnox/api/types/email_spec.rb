...
describe Fortnox::API::Types do
  ...
  context 'when Email created with nil' do
    subject { klass[nil] }

    it { is_expected.to be_nil }
  end

  context 'when Email created with empty string' do
    subject { klass[''] }

    it { is_expected.to eq('') }
  end

  context 'when Email created with valid email' do
    subject { klass[input] }
  ...
  context 'when Email created with more than 1024 characters' do
